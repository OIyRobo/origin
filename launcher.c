...
{
	float counts = 8.673 + (1.372*distance) - (.033*pow(distance, 2)) + .0006*pow(distance, 3);
...
task main()
{
	...
	while(nMotorEncoder[slide] < counts)
	{
		nxtDisplayCenteredTextLine(3, "Encoder: %i", nMotorEncoder[slide]);
		if(counts>counts/2){ //slows down as approaches target
	  	motor[slide] = back;
		}
		else{
			motor[slide] = back-28;
...
//reset
	...
	while(SensorValue(touch) == 0){
		if((encoderFwd-(nMotorEncoder[slide]) > counts/2)){ //slows down as approaches target
	  	motor[slide] = fwd;
		}
		else{
			motor[slide] = fwd+28;
