...
//----------------------------------------------------Driving Code------------------------------------------------------------//
    ...
    float joystick_y1 = joystick.joy1_y1/-1.0;  // renaming the joystick values so they are out of 100 and have the correct sign.
   	...
   	if(abs(joystick.joy1_x1) > threshold || abs(joystick.joy1_y1) > threshold || abs(joystick.joy1_x2) > threshold)
    {
    	...
    	motor[leftFrontMotor]  = leftFrontMotorPower;  // if any joystick is above the threshold the robot moves.
    	...
    	motor[leftFrontMotor]  = 0;  // if not in the loop of any of the joysticks above threshold, the robot stops moving.
    	motor[rightBackMotor]  = 0;
    }
//----------------------------------------------------Flywheel Code-----------------------------------------------------------//
  	/*if(joy1Btn(02) == 1)
    {
     	motor[flywheelMotor] = 100; //move the flyyyyyyyywheeeeeeeeeeeeeeeeel
    }
    else if(joy1Btn(02) == 0)
    {
    	motor[flywheelMotor] = 0; //stop the flyyyyyyyyyyyyyyyyywheeeeeeeeeeeeeeel
    }*/
//-----------------------------------------------------Flag Raiser------------------------------------------------------------//
    if(joy1Btn(03) == 1)
    {
...
//-----------------------------------------------------Hopper Lifter----------------------------------------------------------//
		...
		{
    	motor[lifterMotor1] = 75; //lift the blocks and prepare them to dump
    	motor[lifterMotor2] = 75;
		}
		if(joy1Btn(01) == 1 && nMotorEncoder[lifterMotor1] <= 1755)
		{
			motor[lifterMotor1] = -75; //reverse the motors to put it at the lowered position
    	motor[lifterMotor2] = -75;
		}
		else if(joy1Btn(04) == 0 && joy1Btn(01) == 0)
		{
    	motor[lifterMotor1] = 0; //stop while neither being pressed
    ...
    if(joy1Btn(05) == 1 && nMotorEncoder[lifterMotor2] <= 1755)
    {
    	motor[lifterMotor2] = -75;
    }
    if(joy1Btn(06) == 1 && nMotorEncoder[lifterMotor1] >= 0)
    {
    	motor[lifterMotor1] = 75;
    }
    if(joy1Btn(07) == 1 && nMotorEncoder[lifterMotor2] >= 0)
    {
    	motor[lifterMotor2] = 75;
    }
